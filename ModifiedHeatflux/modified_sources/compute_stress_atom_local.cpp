/* ----------------------------------------------------------------------
LAST_MODIFIED="2019/09/30 20:31:58" 
...
ComputeStressAtomLocal::ComputeStressAtomLocal(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  id_temp(NULL), stress(NULL)
{
  if (narg < 5) error->all(FLERR,"Illegal compute stress/atom/local command");

  peratom_flag = 1;
  size_stress_cols = 9;
  size_peratom_cols = size_stress_cols;
  pressatomflag = 1;
  timeflag = 1;
  comm_reverse = size_stress_cols;

  //store region of control volume
  cvid    = force->cv->register_cv(arg[3]);
  voffset = cvid*9;

  // store temperature ID used by stress computation
  // insure it is valid for temperature computation

  if (strcmp(arg[4],"NULL") == 0) id_temp = NULL;
  else {
    error->all(FLERR,"compute stress/atom/local does not support temperature ID");
//    int n = strlen(arg[4]) + 1;
//    id_temp = new char[n];
//    strcpy(id_temp,arg[4]);
//
//    int icompute = modify->find_compute(id_temp);
//    if (icompute < 0)
//      error->all(FLERR,"Could not find compute stress/atom/local temperature ID");
...
//                 "compute temperature");
  ...
  if (narg == 5) {
    ...
    while (iarg < narg) {
      ...
      } else error->all(FLERR,"Illegal compute stress/atom/local command");
      iarg++;
    }
  }
...
ComputeStressAtomLocal::~ComputeStressAtomLocal()
{
  delete [] id_temp;
...
void ComputeStressAtomLocal::init()
{
...
void ComputeStressAtomLocal::compute_peratom()
{
  int i,j;
  double onemass;
  ...
  if (atom->nmax > nmax) {
    memory->destroy(stress);
    nmax = atom->nmax;
    memory->create(stress,nmax,size_stress_cols,"stress/atom:stress");
    array_atom = stress;
  }
  ...
  if (improperflag && force->improper) {
    ...
    for (i = 0; i < nbond; i++)
      for (j = 0; j < 6; j++) //H.M. Will be modified when necessary
  ...
  for (i = 0; i < nlocal; i++)
    if (!(mask[i] & groupbit)) {
      for(j = 0; j < size_stress_cols; j++)	stress[i][j] = 0.0;
    }
  ...
  if (keflag) {
    ...
    if (biasflag == NOBIAS) { //H.M. kinetic contribution is not touched
      if (rmass) {
        for (i = 0; i < nlocal; i++)
          if (mask[i] & groupbit) {
  ...
  for (i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      for(j = 0; j<size_stress_cols; j++) stress[i][j] *= nktv2p;
    }
}

/* ---------------------------------------------------------------------- */

int ComputeStressAtomLocal::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void ComputeStressAtomLocal::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double ComputeStressAtomLocal::memory_usage()
{
  double bytes = nmax*size_stress_cols * sizeof(double);
  return bytes;
}
